...
# Phenotype - non-competition
p1 <- plot_top_pheno(traditional_supervised, label_text = FALSE, large_fig = TRUE,
                     top_count = 5, title = "Traditional supervised learning")

p2 <- plot_top_pheno(deep_supervised, label_text = FALSE, large_fig = TRUE,      
                     top_count = 5, title = "Deep supervised learning")

p3 <- plot_top_pheno(semi_supervised, label_text = FALSE, large_fig = TRUE,       
                     top_count = 5, title = "Semi-supervised learning")

p4 <- plot_top_pheno(weakly_supervised, label_text = FALSE, large_fig = TRUE,    
                     top_count = 5, title = "Weakly-supervised learning")

p5 <- plot_top_pheno(un_supervised, label_text = FALSE, large_fig = TRUE,        
                     top_count = 5, title = "Unsupervised learning", 
                     string_wrap = 25, ylim = 5, restrict_count = TRUE, 
                     groupvar_stack = "Pheno_cluster_category")
...
g1 <- plot_grid(p1, p2, p3, p4, p5_unlegend, legend, ncol = 2, nrow = 3, align = "v", axis = "l")
ggsave(g1, file="../figure/fig2.eps", width=35, height=40, units="cm", dpi=600) 
```
...
```{r, fig.height=8}
sdoh_unnested <- unnest_two_string(df %>% filter(Pheno_SDOH >= 1), 
                                   c("Phenotype", "Pheno_SDOH_category"), utility = "sdoh")
p1 <- horizontal_bar(sdoh_unnested, "Phenotype", 
               "Pheno_SDOH_category", 
               restrict_count = FALSE, 
               ylim = 8, 
               legend_cap = "SDOH category")

temporal_unnested <- unnest_string_var(df %>% filter(Pheno_temporal >= 1), "Phenotype")
p2 <- horizontal_bar(temporal_unnested, "Phenotype_unnested", 
               "Pheno_temporal_category",
               restrict_count = FALSE, 
               ylim = 2, 
               legend_cap = "Temporal phenotype category")
...
g2 <- plot_data_source(data_source, "Data_type", facet = TRUE, facet_var = "Category", large_fig = TRUE)
ggsave(g2, file="../figure/fig3.eps", width=40, height=20, units="cm", dpi=600) 
```
...
print_tables(method_unnested, "Competition_data_name_unnested", "ML_type", "Traditional", top_count = 10, restric_count = FALSE)
```
...
print_tables(method_unnested, "NLP_software_unnested", "ML_type", "Traditional", top_count = 10, restric_count = FALSE)
```
...
ggsave(g4, file="../figure/fig4.eps", width=30, height=30, units="cm", dpi=600) 
```

## Deep supervised ML vs. supervised ML
...
```{r, eval = F, results='hide'}
g5 <- plot_validate_metrics(deep_supervised, comparator = "deep", large_fig = TRUE)
ggsave(g5, file="../figure/fig5.eps", width=30, height=35, units="cm", dpi=600) 
```
...
print_tables(method_unnested, "Model_performance_metrics_unnested", "ML_type", "Traditional", 
             top_count = 8, restric_count = FALSE, col_width = 4.6)
```
...
cat("There are", sum(df$Reported_demographics == 1, na.rm = T), "papers reported demographcis,", 
    round(sum(df$Reported_demographics == 1, na.rm = T)/nrow(df), 4), "\n") 
cat("There are", sum(df$Open_code == 1, na.rm = T), "papers reported demographcis,", 
    round(sum(df$Open_code == 1, na.rm = T)/nrow(df), 4)) 
```
